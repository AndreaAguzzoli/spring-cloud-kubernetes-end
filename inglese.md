...
- **Volumes** ${\rightarrow}$ A *volume* is an abstraction representing a storage area attached to a pod. Volumes are used to store data generated and utilized by containers and are designed to be shared among containers within the same pod. Their lifecycle is independent of the pod, meaning that even if a pod is restarted, the volume remains intact, and the stored data is still available.

- **Persistent Volumes (PV)** ${\rightarrow}$ A *PV* is a storage resource that is created and configured within the cluster. PVs can be provisioned by an administrator and may represent various types of storage, such as local disks, network storage, or cloud storage.
